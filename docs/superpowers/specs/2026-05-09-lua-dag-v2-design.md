...
| Transition                       | Trigger                                                                                                  |
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `submitted → accepted`           | Ingress validator ký + gossip commitment; có ≥ 1 honest replica xác nhận đã nhận đủ Merkle inclusion proof |
...
- `accepted` và `ordered` **không bao giờ revert** trên 1 node nhất định (monotonic local view), nhưng có thể bị **không-bao-giờ-tiến-tới `soft_confirmed`** nếu anchor không commit được.
- `soft_confirmed → ordered` revert có thể xảy ra **chỉ khi** `lock_macro` rule (§11.5) bị vi phạm hoặc anchor wave bị skip retroactively. Đây là điều kiện hiếm và spec coi như edge-case bug-class.
- `justified → soft_confirmed` revert có thể xảy ra nếu MacroQC bị orphan trong macro fork; trong trường hợp đó rollup phải đợi resolve qua slashing (§11.1).
- `finalized` **chỉ revert** với bằng chứng slashable cho ≥ W/3 stake (Casper FFG safety, §11.1). Đây là failure mode "accountable halt", không phải silent revert.
...
$$\text{UnavailabilityEvidence} = \{\text{Challenge}, \text{TargetSet}, \text{NoResponseProof}_{>2f \text{ custody members}}\}$$

Trong đó `NoResponseProof` = aggregated signed attestations từ ≥ 2f+1 honest non-custody validator confirming "tôi đã observe challenge gossip + đã đợi $T_{\text{retrieve}}$ + không thấy valid response từ target $v_j$".
...
2. **Trong mỗi subnet**, một subnet aggregator (rotated by iVRF) thu chữ ký. Khi đủ 2/3 subnet stake, aggregator publish `subnet_aggregate[k] = (subnet_id, bitmap, bls_aggregate, signed_stake)` kèm bằng chứng đã đạt threshold.
3. **Macro proposer** thu subnet aggregates. Quy tắc accept (cố ý cho phép degraded path):

   **Quorum rule**: macro proposer được phép aggregate `MacroQC` từ **bất kỳ subset nào** của 8 subnet aggregates miễn:
   - $\sum_{k \in S} \text{signed\_stake}_k \ge \lceil 2W/3 \rceil$, VÀ
   - Mỗi `subnet_aggregate[k]` trong $S$ đều hợp lệ (verified bởi proposer).

   Trong điều kiện normal $S = \{0,\dots,7\}$ và tổng stake gần $W$. Khi 1–2 subnet timeout/failed, proposer vẫn có thể đạt $2W/3$ với 6–7 subnet còn lại. Điều này tránh single-subnet failure ⇒ MacroQC failure.

   `MacroQC` lưu lại `included_subnets` bitmap để verifier biết những subnet nào được dùng.
...
3. **Canonical selection** (rev.2): vì nhiều node có thể tạo MacroQC candidate khác nhau (cùng height, cùng `MacroProposal_hash`, nhưng `included_subnets` bitmap khác nhau), spec chọn **canonical MacroQC** = candidate có `included_subnets` bitmap **lex-smallest** (interpret bitmap as 8-bit unsigned integer, smallest wins) trong tất cả các MacroQC candidate hợp lệ thấy được trong window $T_{\text{canonicalize}} = 2 \cdot \text{T\_macropropose}$.
...
4. Mọi validator phải re-broadcast canonical MacroQC khi quan sát thấy candidate nhỏ hơn lex-bitmap so với cái mình đang giữ. Convergence đạt được trong $O(\log N)$ gossip rounds (Long et al. 2019).
